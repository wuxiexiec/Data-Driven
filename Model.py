...
# Read data
mer_information = pd.read_excel('./汇总.xlsx', sheet_name=2)
...
# Coordinates of xz plane
all_x = np.arange(0.125, 26, 0.25)
all_z = np.arange(0.68, 5.32, 0.04)
...
for i in range(0, len(mer_coo)):
    ...
    d_z = np.abs(xz_GCF_coo[:, 1] - mer_coo[i, 1])
...
yz_Nn_Mat = np.empty(shape=(len(yz_all_coo), n_components))
for i in range(0, len(yz_all_coo)):
    ...
    xishu = (1 + 4 * (d_y / sof_y)) * (1 + 4 * (d_z / sof_z)) \
            * np.exp(-4 * ((d_y / sof_y) + (d_z / sof_z)))
    yz_Nn_Mat[i, :] = pca.transform(xishu.reshape(1, -1))
...
yz_mer_Mat = np.empty(shape=(len(mer_coo), n_components))
for i in range(0, len(mer_coo)):
    d_y = np.abs(yz_GCF_coo[:, 0] - mer_coo[i, 0])
    d_z = np.abs(yz_GCF_coo[:, 1] - mer_coo[i, 1])

    xishu = (1 + 4 * (d_y / sof_y)) * (1 + 4 * (d_z / sof_z)) \
            * np.exp(-4 * ((d_y / sof_y) + (d_z / sof_z)))
    yz_mer_Mat[i, :] = pca.transform(xishu.reshape(1, -1))
...
history = model.fit(xunlian_x, xunlian_y, batch_size=25, epochs=500, verbose=0)
